...
import os  # 导入操作系统模块
from tools.wordtopdf import wordtopdf
from tools.wordtopdf import wordtopdf1
from tools.wordtopdf import getPdfOutlines
...
class TransformWindow(QMainWindow, Ui_TransformWindow):
    ...
    def mExecute(self):
        targetpath = self.targetpath.text()  # 获取目标文件夹
        print(f"目标路径: {targetpath}")

        # 获取文件列表（假设你已经在其他地方填充了 `filelist`）
        valueList = wordtopdf(filelist, targetpath)  # 批量转换 Word 文档为 PDF

        if valueList != -1:
            print("转换完成")
            self.showLoding.clear()  # 清除进度条
            self.listpdf.addItems(valueList)  # 将转换后的 PDF 路径显示在目标列表中
        else:
            print("转换失败")
            self.showLoding.clear()  # 清除进度条
            QMessageBox.warning(self, "转换失败", "批量转换过程中发生错误，请重试！", QMessageBox.Ok)
    ...
    def sExecute(self):
        ...
        valueList = wordtopdf(filelist, targetpath)  # 将多个Word文档转换为PDF文件
        if(valueList != -1):
            print("进入到这里了")
...
class ListWindow(QMainWindow, Ui_ListWindow):
    ...
    def getList(self):
        ...
        if(valueList != -1):
            ...
            resultvalue=getPdfOutlines(temp[0],targetpath,isList)   # 提取目录
